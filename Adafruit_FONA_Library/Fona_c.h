...
class Adafruit_FONA : public FONAStreamType {
 public:
  ...
  //GPS (Custom)
  boolean GPSPositioning(void);

  // TCP raw connections (Custom)
  boolean TCPconnect(char *server, char *port);
  boolean TCPclose(void);
  boolean TCPconnected(void);
  boolean TCPsend(char *packet, uint8_t len);
  boolean TCPgpsHello(void);
  boolean TCPgps(void);
  boolean TCPgps(char* mssg,bool flag);
 ...
 protected:
  ...
  FONAFlashStringPtr error_reply;
  // HTTP helpers
